...
namespace StarBurstBloom
{
    ...
    private RTHandle kernelTex;
...
#if false
        private static int kernelSize = 512;
        private static int kernelLog = (int)Mathf.Log(kernelSize, 2);
#else
        private static int kernelSize = 1024;
        private static int kernelLog = (int)Mathf.Log(kernelSize, 2);
#endif
        ...
        private ConvolutionBloom bloom;
        ...
        private static UnityEngine.Experimental.Rendering.GraphicsFormat format = UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat;
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            if (bloom == null)
            {
                bloom = VolumeManager.instance.stack?.GetComponent<ConvolutionBloom>();
                return;
            }
            ...
            {
                passData.threshold = bloom.threshold.value;
            ...
            {
                passData.vertexCount = bloom.bladeCount.value;
                passData.rotation = bloom.rotation.value;
                passData.lensSize = bloom.size.value;
            ...
            using (var builder = renderGraph.AddComputePass("PSF Phase Gen Pass", out GenPhasePassData passData))
            {
                ...
                passData.M = kernelLog;
                passData.N = kernelSize;
                ...
                builder.SetRenderFunc(static (GenPhasePassData passData, ComputeGraphContext context) =>
                {
                    ...
                    context.cmd.SetComputeIntParam(passData.shader, "M", kernelLog);
                    context.cmd.SetComputeIntParam(passData.shader, "N", kernelSize);
            ...
            if (bloomMaterial)
            {
                ...
                {
                    passData.intensity = bloom.intensity.value;
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        {
            colorClipMaterial.SetFloat("_Threshold", bloom.threshold.value);
                Blitter.BlitTexture(cb, cameraColorRT, tempRenderTargetIdentifier, colorClipMaterial, 0);
        }
        else
        {
            colorClipMaterial = new Material(colorClipShader);
            ...
            Blitter.BlitCameraTexture(cb, cameraColorRT, sourceRenderTargetIdentifier);


            if (polygonMaterial != null)
        {
            polygonMaterial.SetInt("_NCount", bloom.bladeCount.value);
            polygonMaterial.SetFloat("_Size", bloom.size.value);
            polygonMaterial.SetFloat("_Theta", bloom.rotation.value);
            polygonMaterial.SetFloat("_Open", 0.0f);

            Blitter.BlitTexture(cb, cameraColorRT, kernelTex, polygonMaterial, 0);
        }
        ...
        {
                int kernelIndex = spectralScaleShader.FindKernel("CSMain");
            cb.SetComputeTextureParam(spectralScaleShader, kernelIndex, "Imag", kernelImagTex1);
            cb.SetComputeTextureParam(spectralScaleShader, kernelIndex, "Real", kernelRealTex1);
            cb.DispatchCompute(spectralScaleShader, kernelIndex, kernelSize / 8, kernelSize / 8, 1);
        }
        ...
        if (bloomMaterial)
        {
            ...
            bloomMaterial.SetFloat("_Intensity", bloom.intensity.value);
                Blitter.BlitTexture(cb, sourceRenderTargetIdentifier, cameraColorRT, bloomMaterial, 0);
        }
        else
        {
            bloomMaterial = new Material(compositionShader);
